    ...
    for (unique_symbol in unique(element$mapped_symbol)) {
        ...
        if(rows$quote.valid[s]==1 && rows$quote.valid[f]==1){
            new_row <- data.frame(
                ...
                quote.bid = rows$quote.bid[s],
                quote.ask = rows$quote.ask[s],
                quote.bidSz = rows$quote.bidSz[s],
                quote.askSz = rows$quote.askSz[s],
                quote.f_mid = rows$quote.mid[f],
                quote.f_bid = rows$quote.bid[f],
                quote.f_ask = rows$quote.ask[f],
                quote.f_bidSz = rows$quote.bidSz[f],
                quote.f_askSz = rows$quote.askSz[f],
                rate = rate_value,
                stringsAsFactors = FALSE
            )
...
get_grouped_data <- function(data, underlying_mapping){
    ...
    data <- data[, c("time_sec", "time", "date", "symbol", "quote.mid", "quote.bid", "quote.ask", "quote.bidSz", "quote.askSz", "quote.valid")]
...
get_position <- function(data, upper_threshold, lower_threshold){
    positions = rep(0, 16)
    for (i in 1:nrow(data)) {
        row <- data[i, ]
        if (row$rate > upper_threshold) {
            positions[i] = row$volume
        }
        if (row$rate < lower_threshold) {
            positions[i] = -row$volume
        }
    }
    return(positions)
