get_thresholds <- function(){
    ...
    # lower_thresholds["USIM5_BOVESPA"] = 9.15
    ...
    lower_thresholds["B3SA3_BOVESPA"] = 9.9
    lower_thresholds["BBAS3_BOVESPA"] = 9.9
    lower_thresholds["BBDC4_BOVESPA"] = 9.9
    lower_thresholds["CSNA3_BOVESPA"] = 9.9
    lower_thresholds["GGBR4_BOVESPA"] = 9.9
    lower_thresholds["HYPE3_BOVESPA"] = 9.9
    lower_thresholds["ITSA4_BOVESPA"] = 9.9
    lower_thresholds["ITUB4_BOVESPA"] = 9.9
    lower_thresholds["JBSS3_BOVESPA"] = 9.9
    lower_thresholds["MGLU3_BOVESPA"] = -100
    lower_thresholds["NTCO3_BOVESPA"] = 9.9
    lower_thresholds["PETR4_BOVESPA"] = 9.9
    lower_thresholds["RENT3_BOVESPA"] = 9.9
    lower_thresholds["USIM5_BOVESPA"] = 9.9
    lower_thresholds["VALE3_BOVESPA"] = 9.9
    lower_thresholds["WEGE3_BOVESPA"] = 9.9

    upper_thresholds["B3SA3_BOVESPA"] = 9.9
    upper_thresholds["BBAS3_BOVESPA"] = 9.9
    upper_thresholds["BBDC4_BOVESPA"] = 9.9
    upper_thresholds["CSNA3_BOVESPA"] = 9.9
    upper_thresholds["GGBR4_BOVESPA"] = 9.9
    upper_thresholds["HYPE3_BOVESPA"] = 9.9
    upper_thresholds["ITSA4_BOVESPA"] = 9.9
    upper_thresholds["ITUB4_BOVESPA"] = 9.9
    upper_thresholds["JBSS3_BOVESPA"] = 9.9
    upper_thresholds["MGLU3_BOVESPA"] = 100
    upper_thresholds["NTCO3_BOVESPA"] = 9.9
    upper_thresholds["PETR4_BOVESPA"] = 9.9
    upper_thresholds["RENT3_BOVESPA"] = 9.9
    upper_thresholds["USIM5_BOVESPA"] = 9.9
    upper_thresholds["VALE3_BOVESPA"] = 9.9
    upper_thresholds["WEGE3_BOVESPA"] = 9.9
...
calc_gross <- function(position, current_instant){
    gross = rep(0, 16)
    for(k in 1:16)
        gross[k] =  abs(position[k]) * (current_instant$quote.mid[k] + 0.1 * current_instant$quote.f_mid[k])
    return(gross)
}

get_profits <- function(new_position, position, current_instant, avg_price){
    profit = rep(0, 16)
    current_diff = current_instant$quote.f_mid - current_instant$quote.mid
    change = abs(new_position - position)
    turnover = change*(current_instant$quote.f_mid*0.1 + current_instant$quote.mid )
    # Vectorized operation for the first condition
    cond1 <- (new_position >= 0 & position <= 0) | (new_position <= 0 & position >= 0)
    profit[cond1] <- profit[cond1] + (avg_price[cond1] - current_diff[cond1]) * position[cond1]
    ...
    cond3 <- (new_position > 0 & position > 0 & new_position <= position) |
            (new_position < 0 & position < 0 & new_position >= position)
    profit[cond3] <- profit[cond3] + (avg_price[cond3] - current_diff[cond3]) * (position[cond3] - new_position[cond3])
    return (list(profit = profit, avg_price = avg_price, turnover = turnover))
}

modify_position <- function(new_position, position, current_instant){
    # if -ve position at 15 min then upper threshold 0 lower threshold 0
    # dont let any position exceed 10L
    # new_position = position + constant_thresholds(current_instant)
    negative_positions = new_position < 0
    new_position[negative_positions] = 0
    if(sum(calc_gross(new_position, current_instant))>MAX_GROSS){
        # liquidate
